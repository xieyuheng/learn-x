...
int main(void) {
  ...
  for (request_nbr = 0; request_nbr < 10; request_nbr++) {
    ...
    s_sendmore(requester,
               ""); // send multi part msg,the first part is empty part
    ...
    // if the first part you received is empty part,then continue receiving next
    // part
    if (strcmp(buffer, "") == 0) {
      memset(buffer, 0, sizeof(buffer));
      len = zmq_recv(requester, buffer, 10, 0);
    ...
    // if the first part you received is not empty part,discard the whole ZMQ
    // msg
    else {
