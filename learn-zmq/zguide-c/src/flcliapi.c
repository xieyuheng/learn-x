...
int server_ping(const char *key, void *server, void *socket) {
...
int server_tickless(const char *key, void *server, void *arg) {
  server_t *self = (server_t *)server;
...
void agent_destroy(agent_t **self_p) {
  assert(self_p);
  if (*self_p) {
    agent_t *self = *self_p;
...
void agent_control_message(agent_t *self) {
...
void agent_router_message(agent_t *self) {
