...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2 && streq(argv[1], "-p")) {
    ...
    self->bstar =
        bstar_new(BSTAR_PRIMARY, "tcp://*:5003", "tcp://localhost:5004");
    ...
    self->bstar =
        bstar_new(BSTAR_BACKUP, "tcp://*:5004", "tcp://localhost:5003");
...
static int s_send_single(const char *key, void *data, void *args) {
  ...
  if (strlen(kvroute->subtree) <= strlen(kvmsg_key(kvmsg)) &&
      memcmp(kvroute->subtree, kvmsg_key(kvmsg), strlen(kvroute->subtree)) ==
          0) {
...
static int s_flush_single(const char *key, void *data, void *args) {
  clonesrv_t *self = (clonesrv_t *)args;
...
static int s_new_active(zloop_t *loop, zmq_pollitem_t *unused, void *args) {
  clonesrv_t *self = (clonesrv_t *)args;
...
static int s_new_passive(zloop_t *loop, zmq_pollitem_t *unused, void *args) {
  clonesrv_t *self = (clonesrv_t *)args;
