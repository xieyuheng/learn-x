...
static void interface_agent(void *args, zctx_t *ctx, void *pipe);
...
interface_t *interface_new(void) {
  interface_t *self;
...
void interface_destroy(interface_t **self_p) {
  assert(self_p);
  if (*self_p) {
...
static zmsg_t *interface_recv(interface_t *self) {
  assert(self);
...
static char *s_uuid_str(uuid_t uuid) {
...
static peer_t *peer_new(uuid_t uuid) {
...
static void peer_destroy(peer_t **self_p) {
  assert(self_p);
  if (*self_p) {
    peer_t *self = *self_p;
    free(self->uuid_str);
...
static byte *peer_uuid(peer_t *self) {
  assert(self);
  return self->uuid;
}

static char *peer_uuid_str(peer_t *self) {
  assert(self);
...
static void peer_is_alive(peer_t *self) {
  assert(self);
...
static void peer_freefn(void *argument) {
...
static agent_t *agent_new(zctx_t *ctx, void *pipe) {
...
static void agent_destroy(agent_t **self_p) {
  assert(self_p);
  if (*self_p) {
    agent_t *self = *self_p;
...
static int agent_control_message(agent_t *self) {
...
static int agent_handle_beacon(agent_t *self) {
  uuid_t uuid;
  ssize_t size = udp_recv(self->udp, uuid, sizeof(uuid_t));

  //  If we got a UUID and it's not our own beacon, we have a peer
  if (size == sizeof(uuid_t) && memcmp(uuid, self->uuid, sizeof(uuid))) {
...
static int agent_reap_peer(const char *key, void *item, void *argument) {
...
static void interface_agent(void *args, zctx_t *ctx, void *pipe) {
  ...
  zmq_pollitem_t pollitems[] = {{self->pipe, 0, ZMQ_POLLIN, 0},
