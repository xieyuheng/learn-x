...
static server_t *server_new(zctx_t *ctx, char *address, int port,
                            char *subtree) {
...
static void server_destroy(server_t **self_p) {
  assert(self_p);
  if (*self_p) {
  ...
  if (streq(command, "SUBTREE")) {
    ...
    if (self->nbr_servers < SERVER_MAX) {
      self->server[self->nbr_servers++] =
          server_new(self->ctx, address, atoi(service), self->subtree);
      //  We broadcast updates to all known servers
      zsocket_connect(self->publisher, "%s:%d", address, atoi(service) + 2);
    } else
...
static void clone_agent(void *args, zctx_t *ctx, void *pipe) {
  ...
  while (true) {
    zmq_pollitem_t poll_set[] = {{pipe, 0, ZMQ_POLLIN, 0},
                                 {0, 0, ZMQ_POLLIN, 0}};
    int poll_timer = -1;
    int poll_size = 2;
    ...
    case STATE_INITIAL:
      ...
      if (self->nbr_servers > 0) {
        zclock_log("I: waiting for server at %s:%d...", server->address,
                   server->port);
    ...
    if (poll_set[0].revents & ZMQ_POLLIN) {
      ...
      if (self->state == STATE_SYNCING) {
        ...
        if (streq(kvmsg_key(kvmsg), "KTHXBAI")) {
          ...
          zclock_log("I: received from %s:%d snapshot=%d", server->address,
                     server->port, (int)self->sequence);
          kvmsg_destroy(&kvmsg);
        } else
          kvmsg_store(&kvmsg, self->kvmap);
        ...
        if (kvmsg_sequence(kvmsg) > self->sequence) {
          self->sequence = kvmsg_sequence(kvmsg);
          kvmsg_store(&kvmsg, self->kvmap);
          zclock_log("I: received from %s:%d update=%d", server->address,
                     server->port, (int)self->sequence);
        } else
          kvmsg_destroy(&kvmsg);
      }
      ...
      zclock_log("I: server at %s:%d didn't give HUGZ", server->address,
                 server->port);
